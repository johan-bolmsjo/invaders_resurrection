...
static uint8_t
pixel_get_alpha(struct Image* image, int x, int y)
{
    if (image->type != RGBA) {
        return 0;
    }
...
static int
hline_same_alpha(struct Image* image, int line, uint8_t alpha)
{
    int i;
...
static int
vline_same_alpha(struct Image* image, int line, uint8_t alpha)
{
    int i;
...
void
clip_auto_alpha(struct Image* image, struct Clip* clip)
{
