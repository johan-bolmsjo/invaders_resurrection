...
static DefFile d_file; /* En liten fuuuling:) */
...
cmd_frame(DefObject* d_obj)
{
    ...
    frames = d_obj->frames + 1;
    ipp2 = realloc(d_obj->ipp, frames * sizeof(Image**));
    if (ipp2 == NULL)
        return E_MEM;
...
static int
cmd_rgb_to_alpha(DefObject* d_obj)
{
    int i, v;
    uint8_t rgb[3], thres[3] = {0, 0, 0};
    char* p;
    ...
    for (i = 0; i < 3; i++) {
        v = ascii_hex_to_int(p + i * 2);
        if (v < 0)
            return ERROR;
        rgb[i] = v;
    }

    for (i = 0; i < d_obj->frames; i++)
...
static int
cmd_autocrop_alpha(DefObject* d_obj)
{
    int i;
    Clip clip;
    Image* image;
    ...
    for (i = 0; i < d_obj->frames; i++) {
        image = crop(d_obj->ipp[i], &d_obj->clip);
        if (image == NULL)
            return ERROR;
...
/* Scales all images in d_obj to fit in the specified window size,
 * keeps the aspect ratio.
 */

static int
cmd_scale_maxspect(DefObject* d_obj)
{
    int i, xy[2];
    double aspx, aspy;
    char* p;
    ...
    if (d_obj->clip.w != d_obj->clip.h) {
        aspx = (double)d_obj->clip.w / xy[0];
        aspy = (double)d_obj->clip.h / xy[1];
        if (aspx > aspy)
    ...
    for (i = 0; i < d_obj->frames; i++) {
        image = image_create(RGBA, 0, xy[0], xy[1]);
        if (image == NULL)
            return ERROR;
...
static int
cmd_end(DefObject* d_obj)
{
    int i, alpha = 0, gfx = 0, cm = 0;
    char* p;
    Clip clip;
    GfxFrame* f;
    Image* image;
    ...
    for (i = 0; i < d_obj->frames; i++) {
        ...
        f = gfx_frame_create(alpha | gfx | cm,
        ...
        if (gfx) {
            if (!alpha) {
                image = colour_rgba_to_rgb(d_obj->ipp[i]);
                if (image == NULL)
                    return ERROR;
...
def_run(const char* filename)
{
    ...
    char* cmd;
    DefObject d_obj = {0};
    DefFunc funcs[] = {{"object", cmd_object},
                       {"frame", cmd_frame},
